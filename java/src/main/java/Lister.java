        ...
        if (musicDir.exists() && musicDir.isDirectory()) {
            ...
            if (Objects.requireNonNull(albums).length != 0) {
                ...
                for (String album:albums){
                    resp.getWriter().println("<p style=\"font-size:1em\"><a href=http://jetson:8080/player?album=" + musicPath + "/" + album.replace(" ","%20") + ">" + album.replace("fuckingslash","/").replace("fuckingquestion","?") + "</a></p>");
                }
