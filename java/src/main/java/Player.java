...
public class Player extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) {
        ...
        try {
            ...
            resp.getWriter().println("<iframe hidden id=tracks width=400 height=430 style=position:fixed;top:120px;right:20px;></iframe>");
        } catch (IOException e) {
            e.printStackTrace();
        }
        albums.forEach((album, albumPath) -> {
            try {
                resp.getWriter().println("<p style=padding-left:280px;color:black;font-size:150%; onclick=gettracks(\"" + (albumPath + "/" + album).replace(" ", "&") + "\")><b>" +
